...
def create_report(cf_id):
    try:
        ...
        if not reason:
            return jsonify({'success': False, 'message': '요청 사유를 입력해주세요.'}), 400
        
        if len(reason) > 500:
            return jsonify({'success': False, 'message': '요청 사유는 500자 이내로 입력해주세요.'}), 400
        
        if db.report.report_exists_for_cf_id(cf_id):
            return jsonify({'success': False, 'message': '이미 비공개 처리가 요청된 콘텐츠입니다.'}), 409
        
        reporter_ip = utils.get_ip()
        
        if db.report.report_create(cf_id, reason, reporter_ip):
            return jsonify({'success': True, 'message': '비공개 처리가 요청되었습니다.'}), 200
        else:
